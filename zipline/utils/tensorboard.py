...
def make_logdir(*sub_dir):
    home_dir = os.path.expanduser('~')
    path = os.path.join(home_dir, 'logdir', *sub_dir)
...
class TensorBoard(object):
    ...
    Args:
        log_dir_args: 可迭代str
            子目录名称。
            如输入`('a','b')`
            默认根目录：root = os.path.expanduser('~')
            日志目录：root/logdir/a/b
    ...
    """
    def __init__(self, log_dir=r'C:\Users\liude', max_queue=10, flush_secs=120):
