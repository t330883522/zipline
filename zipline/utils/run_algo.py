...
def _run(handle_data,
    ...
    if algotext is not None:
        ...
        raise _RunAlgoError('cannot pass define without `algotext`',
    ...
    if bundle is not None:
        ...
        prefix, connstr = re.split(r'sqlite:///',
            str(bundle_data.asset_finder.engine.url),
            maxsplit=1,)
        if prefix:
            raise ValueError("invalid url %r, must begin with 'sqlite:///'" % str(bundle_data.asset_finder.engine.url),)
        #env = TradingEnvironment(asset_db_path=connstr, environ=environ)
        # # 在此传入benchmark参数
        env = TradingEnvironment(asset_db_path=connstr, environ=environ,
                                 bm_symbol = bm_symbol)
        first_trading_day = bundle_data.equity_minute_bar_reader.first_trading_day
        data = DataPortal(env.asset_finder, get_calendar(),
...
def run_algorithm(start,
    ...
    if not non_none_data:
        ...
        raise ValueError('must specify one of `data`, `data_portal`, or `bundle`,'
            ' got: %r' % non_none_data,)

    elif 'bundle' not in non_none_data and bundle_timestamp is not None:
        raise ValueError('cannot specify `bundle_timestamp` without passing `bundle`',)
